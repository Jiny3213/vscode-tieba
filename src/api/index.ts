...
export function getThreadList(
  ...
  return axios
    ...
    .then((res) => {
      ...
      const re =
        /Bigpipe.register\("frs-list\/pagelet\/thread_list", {"content":(".*"),"parent"/;
...
export function getPostList(
  url: string = "https://tieba.baidu.com/p/7029367562",
  page: number = 1
) {
  ...
  return axios
    ...
    .then(async (res) => {
      ...
      if ($("title").text() === "百度安全验证") {
        ...
        return {
          errMessage: "触发百度安全验证，请打开浏览器验证，并重新获取cookie",
        };
      }
...
export function getCommentList(
  ...
  return axios
    .get("https://tieba.baidu.com/p/totalComment", {
      params: {
        t: new Date().getTime(),
        tid: tid, // thread id
    ...
    .then((res) => {
      ...
      for (let item of Object.values(commentList)) {
        (item as CommentItem).comment_info.forEach((commentInfo) => {
          ...
          commentInfo.content = commentInfo.content.replace(
            /<a.*?>(.*?)<\/a>/,
            "<span>$1<span>"
          ); // 去除回复用户高亮
        });
...
export function getMyForum() {
  return axios.get("https://tieba.baidu.com").then((res) => {
    const html = res.data;
    const re =
      /<script>(?:(?!<\/script).)*spage\/widget\/AsideV2.*?(\[\[\{.*?\}\]\]).*?<\/script>/;
